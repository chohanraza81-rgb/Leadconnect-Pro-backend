...
async function sendEmail({ to, subject, html }) {
  ...
  // 1. Try Brevo (Sendinblue) first – most reliable
  if (config.brevoApiKey) {
    ...
    // Use Gmail as sender (must be verified in Brevo) or fallback to a generic address
  ...
  // 2. Fallback to Gmail SMTP (if Brevo not configured)
  ...
  throw new Error(
    'No email credentials configured. Add Brevo API key or Gmail credentials in Settings.'
  );
}
